...
def add_pose(graph, initial_estimate):
    ...
    rot1 = gtsam.Pose2(0,0,np.pi/4)
    move = gtsam.Pose2(2,0,0)
    rot2 = gtsam.Pose2(0,0,np.pi/4)

    totalmove = rot1.compose(move).compose(rot2)
    graph.add(gtsam.BetweenFactorPose2(X(3), X(4), totalmove, ODOMETRY_NOISE))
